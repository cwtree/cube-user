...
public class PhoenixUserManagerImpl implements PhoenixUserManager {
	...
	public PageInfo<PhoenixUser> queryList(String username, String phone, int currentPage) {
		...
		if (currentPage >= 0) {
			// 设置分页信息，排序
			return PageHelper.startPage(currentPage, PAGE_SIZE).setOrderBy("name asc")
					.doSelectPageInfo(() -> phoenixUserMapper.selectByExample(example));
		}
		return new PageInfo<PhoenixUser>(phoenixUserMapper.selectByExample(example));
	}
